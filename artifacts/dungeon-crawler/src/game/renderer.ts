...
function batAnim(enemy: Enemy): 'idle' | 'run' | 'attack' | 'hurt' {
  if (enemy.damageFlashTime > 0) return 'hurt';
  if (Date.now() - enemy.lastAttackTime < enemy.attackCooldown * 0.35) return 'attack';
  if (enemy.aggro) return 'run';
  return 'idle';
}
